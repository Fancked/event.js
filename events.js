(function() {
	...
	window.getEventsByTarget = function(target, simplify = true, origin = events) {
		...
		function dealresult(targets) {
			...
			let map = new Map();
			temp.forEach(x => {
				let fit = [...map.keys()].filter(y => y.type === x.type && y.event === x.event && y.useCapture === x.useCapture);
				if (fit.length !== 0) {
	...
	window.getEventsByType = function(type, simplify = true, origin = events) {
		let result = clone(origin)
		if (type) {
	...
	window.getEventsByEvent = function(event, simplify = true, origin = events) {
		let result = clone(origin);
		if (event) {
	...
	window.getEvents = function(obj, simplify = true, origin = events) {
		let result = clone(origin);
	...
	Element.prototype.getAllEvents = function(simplify = true) {
		return window.getEvents({
			target: this
		}, simplify);
	}
	Element.prototype.getEventsByType = function(type, simplify = true) {
		return window.getEvents({
			type,
			target: this
		}, simplify);
	}
	Element.prototype.getEventsByEvent = function(event, simplify = true) {
		return window.getEvents({
			event,
			target: this
		}, simplify);
	}
	Element.prototype.getEvents = function(obj, simplify = true) {
		return window.getEvents({
			...obj,
			target: this
		}, simplify);
